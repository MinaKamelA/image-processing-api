...
const resizeImage = async (inFilePath: string, outFilePath: string, width: number, height: number): Promise <Buffer> =>{
...
images.get('/', async (req, res) =>{
    if(cache.has(req.originalUrl)){
        ...
        try{
            const imageBuffer = await resizeImage(inPath, outPath, width, height)
